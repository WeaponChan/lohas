...
- (CGFloat)tableView:(UITableView *)mtableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return [HotelSuperCell Height:[dataList objectAtIndex:indexPath.row]];
}
//设置某个cell对象
- (UITableViewCell *)tableView:(UITableView *)mtableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (dataList.count == 0) {
        ...
        cell=[MSUIUtils newCellByNibName: mtableView  NibName:@"HotelSuperCell"];
        HotelSuperCell *mHotelSuperCell = (HotelSuperCell *)cell;
        NSDictionary *item = [dataList objectAtIndex:indexPath.row];
        mHotelSuperCell.sdate=self.sdate;
        mHotelSuperCell.edate=self.edate;
        [mHotelSuperCell update:item Parent:parent ];
    }
