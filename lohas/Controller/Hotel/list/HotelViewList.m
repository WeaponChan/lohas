...
@implementation HotelViewList
@synthesize countryInfo,selectID,comment_numDic,photoList,HeadDic,array,isHideMore;
...
-(void) asyncLoadData
{
    ...
    switch (actionType) {
        ...
        case ACTION_TYPE_PULLREFRESH:{
                ...
                if([parent isKindOfClass:[HotelViewController class]]){
                    HotelViewController *viewCtrl=(HotelViewController*)parent;
                    [viewCtrl refresh];
                }
...
- (CGFloat)tableView:(UITableView *)mtableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (type.intValue) {
        case 0:
            ...
            return [HotelTitleCell Height:item];
            
            break;
            
        case 1:
            
            return [HotelInfoCell Height:item];
            
            break;
            
        case 2:
            
            return [HotelTextCell Height:countryInfo isHide:isHideMore];
...
- (UITableViewCell *)tableView:(UITableView *)mtableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (dataList.count == 0) {
        ...
        switch (type.intValue) {
            case 0:
            {
                cell=[MSUIUtils newCellByNibName: mtableView  NibName:@"HotelTitleCell"];
                
                HotelTitleCell *mHotelTitleCell = (HotelTitleCell *)cell;
                mHotelTitleCell.responseItem=countryInfo;
                mHotelTitleCell.ListItem=HeadDic;
                [mHotelTitleCell update:item Parent:parent];
                
                break;
            }
            case 1:
            {
                cell=[MSUIUtils newCellByNibName: mtableView  NibName:@"HotelInfoCell"];
                
                HotelInfoCell *mHotelInfoCell = (HotelInfoCell *)cell;
                mHotelInfoCell.responseItem=countryInfo;
                [mHotelInfoCell update:item Parent:parent];
                
                break;
            }
            case 2:
            {
                cell=[MSUIUtils newCellByNibName: mtableView  NibName:@"HotelTextCell"];
                
                HotelTextCell *mHotelTextCell = (HotelTextCell *)cell;
                mHotelTextCell.responseItem=countryInfo;
                mHotelTextCell.isHideMore=isHideMore;
                [mHotelTextCell update:item Parent:parent];
