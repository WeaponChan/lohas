...
@implementation newCityList
@synthesize postList,ABarray;
...
- (CGFloat)tableView:(UITableView *)mtableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (ename) {
        return [CityStateCell Height:[dataList objectAtIndex:indexPath.row]];
    }
    else{
        return [CityProvinceCell Height:[dataList objectAtIndex:indexPath.row]];
    }
...
- (UITableViewCell *)tableView:(UITableView *)mtableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (dataList.count == 0) {
        ...
        if (ename) {
            ...
            cell=[MSUIUtils newCellByNibName: mtableView  NibName:@"CityStateCell"];
            CityStateCell *mFoodListCell = (CityStateCell *)cell;
            NSDictionary *item = [dataList objectAtIndex:indexPath.row];
            [mFoodListCell update:item Parent:parent];
        }
        else{
            
            cell=[MSUIUtils newCellByNibName: mtableView  NibName:@"CityProvinceCell"];
            CityProvinceCell *mFoodListCell = (CityProvinceCell *)cell;
            NSDictionary *item = [dataList objectAtIndex:indexPath.row];
            [mFoodListCell update:item Parent:parent];
        }
