...
- (CGFloat)tableView:(UITableView *)mtableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return [WrongCell Height:[dataList objectAtIndex:indexPath.row]];
}
//设置某个cell对象
- (UITableViewCell *)tableView:(UITableView *)mtableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (dataList.count == 0) {
        ...
        cell=[MSUIUtils newCellByNibName: mtableView  NibName:@"WrongCell"];
        WrongCell *mFoodListCell = (WrongCell *)cell;
...
-(void)selectWrong:(NSString*)wrongID{
    ...
    for (NSMutableDictionary *dic in array) {
        NSString *id=[dic objectForKeyNotNull:@"id"];
        if ([id isEqual:wrongID]) {
