...
#import "UserHelpList.h"
#import "UserHelpCell.h"
#import "UserRecommendFriendCell.h"

@implementation UserHelpList{
    int viewTag;//1:帮助列表 2:推荐用户
}
...
- (CGFloat)tableView:(UITableView *)mtableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return [UserHelpCell Height:[dataList objectAtIndex:indexPath.row]];
}
//设置某个cell对象
- (UITableViewCell *)tableView:(UITableView *)mtableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (dataList.count == 0) {
        ...
        if (viewTag==1) {
            cell=[MSUIUtils newCellByNibName: mtableView  NibName:@"UserHelpCell"];
            UserHelpCell *mUserHelp = (UserHelpCell *)cell;
            NSDictionary *item = [dataList objectAtIndex:indexPath.row];
            [mUserHelp update:item Parent:parent];
        }
        else if (viewTag==2){
            cell=[MSUIUtils newCellByNibName: mtableView  NibName:@"UserRecommendFriendCell"];
            UserRecommendFriendCell *mFoodListCell = (UserRecommendFriendCell *)cell;
            NSDictionary *item = [dataList objectAtIndex:indexPath.row];
