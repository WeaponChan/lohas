...
-(void) asyncLoadData
{
    ...
    Api* api = [[Api alloc] init:self tag:@"collectList"];
    [api collectList:pageIndex limit:pageSize lat:[NSString stringWithFormat:@"%f",lat] lng:[NSString stringWithFormat:@"%f",lng]];
...
{
    NSMutableArray *items=[response objectForKeyNotNull:@"items"];

    if (selectList.count==0) {
        for (NSMutableDictionary *dic in items) {
        ...
        for (NSMutableDictionary *dic in items) {
            ...
            for (NSMutableDictionary *diction in selectList) {
                
                NSString *id=[diction objectForKeyNotNull:@"id"];
                if ([id isEqual:subid]) {
...
- (CGFloat)tableView:(UITableView *)mtableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return [TripAddInfoCell Height:[dataList objectAtIndex:indexPath.row]];
}
//设置某个cell对象
- (UITableViewCell *)tableView:(UITableView *)mtableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (dataList.count == 0) {
        ...
        cell=[MSUIUtils newCellByNibName: mtableView  NibName:@"TripAddInfoCell"];
        TripAddInfoCell *mEventListCell = (TripAddInfoCell *)cell;
        NSDictionary *item = [dataList objectAtIndex:indexPath.row];
        mEventListCell.selectList=selectList;
