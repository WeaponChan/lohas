...
#import "UserBlackCell.h"
#import "UserBlackDeleteCell.h"

@implementation UserBlackList
@synthesize isDelete;
...
-(void) asyncLoadData
{
    ...
    Api* api = [[Api alloc] init:self tag:@"blackList"];
    [api blackList:pageIndex limit:pageSize];
...
- (CGFloat)tableView:(UITableView *)mtableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return [UserBlackCell Height:[dataList objectAtIndex:indexPath.row]];
}
//设置某个cell对象
- (UITableViewCell *)tableView:(UITableView *)mtableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (dataList.count == 0) {
        ...
        if (!isDelete) {
            cell=[MSUIUtils newCellByNibName: mtableView  NibName:@"UserBlackCell"];
            UserBlackCell *mEventListCell = (UserBlackCell *)cell;
            NSDictionary *item = [dataList objectAtIndex:indexPath.row];
            [mEventListCell update:item Parent:parent];
        }else{
            
            cell=[MSUIUtils newCellByNibName: mtableView  NibName:@"UserBlackDeleteCell"];
            UserBlackDeleteCell *mEventListCell = (UserBlackDeleteCell *)cell;
            NSDictionary *item = [dataList objectAtIndex:indexPath.row];
            [mEventListCell update:item Parent:parent];
        }
