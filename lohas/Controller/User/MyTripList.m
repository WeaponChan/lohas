...
-(void) asyncLoadData
{
    ...
    Api* api = [[Api alloc] init:self tag:@"journeyList"];
    [api journeyList:pageIndex limit:pageSize];
...
-(void)loaded:(id)response tag:(NSString*)tag
{
    ...
    [items insertObject:dic atIndex:0];
...
- (CGFloat)tableView:(UITableView *)mtableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *type=[dic objectForKeyNotNull:@"type"];
    if (type.intValue==1) {
        return [TripAddCell Height:[dataList objectAtIndex:indexPath.row]];
    }
...
- (UITableViewCell *)tableView:(UITableView *)mtableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (dataList.count == 0) {
        ...
        NSString *type=[dic objectForKeyNotNull:@"type"];
        if (type.intValue==1) {
            cell=[MSUIUtils newCellByNibName: mtableView  NibName:@"TripAddCell"];
            TripAddCell *mEventListCell = (TripAddCell *)cell;
