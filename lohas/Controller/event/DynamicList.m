...
-(void) asyncLoadData
{
    ...
    if (keyword.length>0) {
        ...
        [api searchSaidList:keyword page:pageIndex limit:pageSize];
    }
    else{
        Api* api = [[Api alloc] init:self tag:@"saidListByAttention"];
        [api saidListByAttention:pageIndex limit:pageSize];
    }
...
- (CGFloat)tableView:(UITableView *)mtableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return [DynamicCell Height:[dataList objectAtIndex:indexPath.row]];
}
//设置某个cell对象
- (UITableViewCell *)tableView:(UITableView *)mtableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (dataList.count == 0) {
        ...
        cell=[MSUIUtils newCellByNibName: mtableView  NibName:@"DynamicCell"];
        DynamicCell *mEventListCell = (DynamicCell *)cell;
