...
@implementation DynamicJubaoList
@synthesize jubao_id;
...
-(void) asyncLoadData
{
    ...
    Api* api = [[Api alloc] init:self tag:@"getReportCategory"];
...
{
    for (NSMutableDictionary *diction in response) {
        
        NSString *id=[diction objectForKeyNotNull:@"id"];
        if ([jubao_id isEqualToString:id]) {
            [diction setObject:@"YES" forKey:@"selected"];
        }
        else{
            [diction setObject:@"NO" forKey:@"selected"];
        }
...
- (CGFloat)tableView:(UITableView *)mtableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return [DynamicJubaoCell Height:[dataList objectAtIndex:indexPath.row]];
}
//设置某个cell对象
- (UITableViewCell *)tableView:(UITableView *)mtableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (dataList.count == 0) {
        ...
        cell=[MSUIUtils newCellByNibName: mtableView  NibName:@"DynamicJubaoCell"];
        DynamicJubaoCell *mEventListCell = (DynamicJubaoCell *)cell;
