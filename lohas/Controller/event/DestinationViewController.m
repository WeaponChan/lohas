...
@interface DestinationViewController ()<reviewDelegate>{
    BOOL isBack;
    NSArray *wrongList;
...
-(void)reloadcomment_num{
    ...
    [api comment_num:[responseItem objectForKeyNotNull:@"id"] type:7];
}
...
-(void)loaded:(id)response tag:(NSString*)tag
{
    ...
    if ([tag isEqual:@"get_shopping_info"]) {
        ...
        self.mDestinationList.isDestination=YES;
        self.mDestinationList.array=array;
        self.mDestinationList.infoItem=response;
        self.mDestinationList.responseItem=responseItem;
        self.mDestinationList.comment_numDic=comment_numDic;
        self.mDestinationList.seID=[responseItem objectForKeyNotNull:@"id"];
        if (isBack) {
            [self.mDestinationList refreshData];
        }else{
            self.mDestinationList.isHideMore=YES;
            [self.mDestinationList initial:self];
        }
        ...
        [api destinationInfo:[responseItem objectForKeyNotNull:@"id"]];
    }
        ...
        NSString *SUrl=[NSString stringWithFormat:@"%@?type=7&invite_tel=%@&id=%@",url,phone,[responseDic objectForKeyNotNull:@"id"]];
...
-(void)pickerCallback:(UIImage *)img
{
    ...
    [api sibmit_photo:[responseItem objectForKeyNotNull:@"id"] type:7 avatar:img];
    
}

-(void)getMore:(BOOL)isHide{
    if (isHide) {
        self.mDestinationList.isHideMore=NO;
    }else{
        self.mDestinationList.isHideMore=YES;
    }
