...
@synthesize type,title,cityID,categoryID,lat,lng;
...
-(void) asyncLoadData
{
    ...
    [api get_shopping_lists:pageIndex count:pageSize category_id:categoryID title:title city:cityID lat:lat lng:lng type:type price_type:@"0"];
...
- (CGFloat)tableView:(UITableView *)mtableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return [ShopListCell Height:[dataList objectAtIndex:indexPath.row]];
}
//设置某个cell对象
- (UITableViewCell *)tableView:(UITableView *)mtableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (dataList.count == 0) {
        ...
        ShopListCell *mShopListCell = (ShopListCell *)cell;
        NSDictionary *item = [dataList objectAtIndex:indexPath.row];
        
        [mShopListCell update:item Parent:parent ];
    }
