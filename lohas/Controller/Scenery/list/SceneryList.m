...
@synthesize type,title,cityID,priceType,category,lat,lng;
...
-(void) asyncLoadData
{
    ...
    [api get_travel_lists:pageIndex count:pageSize title:title city_id:cityID lat:lat lng:lng type:type price_type:priceType category:category];
}
...
- (CGFloat)tableView:(UITableView *)mtableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return [SceneryListCell Height:[dataList objectAtIndex:indexPath.row]];
}
//设置某个cell对象
- (UITableViewCell *)tableView:(UITableView *)mtableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (dataList.count == 0) {
        ...
        SceneryListCell *mSceneryListCell = (SceneryListCell *)cell;
        NSDictionary *item = [dataList objectAtIndex:indexPath.row];
        
        [mSceneryListCell update:item Parent:parent ];
    }
