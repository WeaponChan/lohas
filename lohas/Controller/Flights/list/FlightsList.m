...
- (CGFloat)tableView:(UITableView *)mtableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return [FlightsListCell Height:[dataList objectAtIndex:indexPath.row]];
}
//设置某个cell对象
- (UITableViewCell *)tableView:(UITableView *)mtableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (dataList.count == 0) {
        ...
        cell=[MSUIUtils newCellByNibName: mtableView  NibName:@"FlightsListCell"];
        FlightsListCell *mFlightsListCell = (FlightsListCell *)cell;
        NSDictionary *item = [dataList objectAtIndex:indexPath.row];
        [mFlightsListCell update:item Parent:parent];
    }
