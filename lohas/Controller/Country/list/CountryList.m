...
@synthesize type,cityID,typeID,text,category_id,priceType,lat,lng;
...
-(void) asyncLoadData
{
    ...
    [api get_Country_lists:type city:cityID page:pageIndex count:pageSize category_id:category_id title:text lat:lat lng:lng  price_type:priceType];
...
- (CGFloat)tableView:(UITableView *)mtableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return [CountryListCell Height:[dataList objectAtIndex:indexPath.row]];
}
//设置某个cell对象
- (UITableViewCell *)tableView:(UITableView *)mtableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (dataList.count == 0) {
        ...
        CountryListCell *mCountryListCell = (CountryListCell *)cell;
        NSDictionary *item = [dataList objectAtIndex:indexPath.row];
        
        [mCountryListCell update:item Parent:parent ];
    }
