...
@implementation CountryTypeList
...
-(void) asyncLoadData
{
    ...
    if ([self.categoryStr isEqual:@"shop"]) {
        Api *api=[[Api alloc]init:self tag:@"get_shopping_category_lists"];
        [api get_shopping_category_lists];
    }else{
        Api *api=[[Api alloc]init:self tag:@"get_country_category_lists"];
        [api get_country_category_lists];
    }
...
- (CGFloat)tableView:(UITableView *)mtableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return [CountryTypeCell Height:[dataList objectAtIndex:indexPath.row]];
}
//设置某个cell对象
- (UITableViewCell *)tableView:(UITableView *)mtableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (dataList.count == 0) {
        ...
        cell=[MSUIUtils newCellByNibName: mtableView  NibName:@"CountryTypeCell"];
        CountryTypeCell *mCountryListCell = (CountryTypeCell *)cell;
