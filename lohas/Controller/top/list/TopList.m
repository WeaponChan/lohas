...
-(void) asyncLoadData
{
    ...
    Api *api=[[Api alloc]init:self tag:@"get_top_lists"];
    [api get_top_lists:pageIndex count:pageSize city_id:cityId];
...
- (CGFloat)tableView:(UITableView *)mtableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return [TopListCell Height:[dataList objectAtIndex:indexPath.row]];
}
//设置某个cell对象
- (UITableViewCell *)tableView:(UITableView *)mtableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (dataList.count == 0) {
        ...
        cell=[MSUIUtils newCellByNibName: mtableView  NibName:@"TopListCell"];
        TopListCell *mTopListCell = (TopListCell *)cell;
        NSDictionary *item = [dataList objectAtIndex:indexPath.row];
        [mTopListCell update:item Parent:parent];
    }
