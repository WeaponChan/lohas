...
- (id)initWithTitle:(NSString *)title message:(NSString *)message 
{
    ...
    self = [self initWithTitle:title message:message usingInterfaceOrientation:orientation];
    if( self != nil )
    {
    }
...
//            [labelView release];
            ...
            _height += size.height + kAlertViewBorder + 2;
        }
...
{    
    ...
    for (NSUInteger i = 0; i < _blocks.count; i++)
    {
        ...
        CGFloat maxHalfWidth = floorf((_view.bounds.size.width-kAlertViewBtnBorder*3)*0.5);
        CGFloat width = _view.bounds.size.width-kAlertViewBtnBorder*2;
        CGFloat xOffset = kAlertViewBtnBorder;
        if (isSecondButton)
        {
            width = maxHalfWidth;
            xOffset = width + kAlertViewBtnBorder * 2;
            isSecondButton = NO;
        }
        else if (i + 1 < _blocks.count)
        {
            ...
            CGSize size = [title sizeWithFont:buttonFont 
                                  minFontSize:10 
                               actualFontSize:nil
                                     forWidth:_view.bounds.size.width-kAlertViewBtnBorder*2 
                                lineBreakMode:UILineBreakModeClip];
            
            if (size.width < maxHalfWidth - kAlertViewBtnBorder)
            {
                ...
                if (size.width < maxHalfWidth - kAlertViewBtnBorder && needSecondButton)
                {
                    // They'll fit!
        ...
        else if (_blocks.count  == 1)
        {
            ...
            if (size.width + 2 * kAlertViewBtnBorder < width)
            {
                width = size.width + 2 * kAlertViewBtnBorder;
        ...
        if (!isSecondButton)
            _height += kAlertButtonHeight + kAlertViewBtnBorder;
        
        index++;
    }
    
    _height;  // Margin for the shadow
    ...
    NSLog(@"_view.frame.size：%f;%f",frame.size.width,frame.size.height);
...
//    [modalBackground release];
    ...
    _view.layer.shadowOffset  = CGSizeMake(0, 10);
    _view.layer.shadowRadius  = 40;
...
//        [_backgroundImage release];
    ...
    BlockBackground *window = [BlockBackground sharedInstance];
    
    window.vignetteBackground = _vignetteBackground;
    [window addToMainWindow:_view];
