...
- (NSData *)readDataOfLength:(NSUInteger)length
{
    ...
    //NSLog(@"readDataOfLength:%d", result);
	// Check the results
	...
	else // (result > 0)
	{
        ...
        if(mKey) {
            int codelen = mKey.length;
            
            int x = (int) (fileOffset % codelen);
            for(int i=0;i<result;i++) {
                int offset = [mKey characterAtIndex:x];
                
                //if(fileOffset<20) NSLog(@"offset:%d", offset);
                
                if(++x>=codelen) x=0;
                *(unsigned char*)(buffer+i) -= offset;
            }
        }
