...
#import "HistoryUtils2.h"
#import "DBConnection.h"
#import "SBJson.h"

@implementation HistoryUtils2
...
+(BOOL) isHistory:(NSString*)key
{
    ...
    NSString* sql = [NSString stringWithFormat:@"SELECT content FROM history2 Where key='%@' limit 1", key];
...
+(BOOL) setHistory:(NSString*)key content:(NSString*)content
{
    ...
    NSLog(@"save:%@", argsDict);
    [db executeUpdate:@"replace INTO history2 (key,content, createdAt) VALUES (:key,:content,:createdAt)" withParameterDictionary:argsDict];
...
+(BOOL) delHistory:(NSString*)key
{
    ...
    FMDatabase *db  = [DBConnection getSharedDatabase];
    NSString* sql = [NSString stringWithFormat:@"delete FROM history2 Where key='%@'", key];
    //NSLog(@"sql:%@", sql);
...
+(BOOL) clearHistory
{
    ...
    FMDatabase *db  = [DBConnection getSharedDatabase];
    NSString* sql = [NSString stringWithFormat:@"delete FROM history2"];
    //NSLog(@"sql:%@", sql);
    [db executeUpdate:sql];
...
+(NSArray*) getHistory:(int)limit offset:(int)offset
{
    ...
    NSString* sql = [NSString stringWithFormat:@"SELECT * FROM history2 order by createdAt desc limit %d offset %d",  limit, offset];
