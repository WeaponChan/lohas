...
+(BOOL) isFav:(NSString*)key
{
    ...
    NSString* sql = [NSString stringWithFormat:@"SELECT content FROM fav Where key='%@' limit 1", key];
...
+(BOOL) setFav:(NSString*)key content:(NSString*)content
{
    ...
    //NSLog(@"save:%@", argsDict);
    [db executeUpdate:@"replace INTO fav (key,content, createdAt) VALUES (:key,:content,:createdAt)" withParameterDictionary:argsDict];
...
+(BOOL) delFav:(NSString*)key
{
    ...
    FMDatabase *db  = [DBConnection getSharedDatabase];
    NSString* sql = [NSString stringWithFormat:@"delete FROM fav Where key='%@'", key];
    //NSLog(@"sql:%@", sql);
...
+(BOOL) clearFav
{
    ...
    FMDatabase *db  = [DBConnection getSharedDatabase];
    NSString* sql = [NSString stringWithFormat:@"delete FROM fav"];
    //NSLog(@"sql:%@", sql);
    [db executeUpdate:sql];
...
+(NSArray*) getFav:(int)limit offset:(int)offset
{
    ...
    NSString* sql = [NSString stringWithFormat:@"SELECT * FROM fav order by createdAt desc limit %d offset %d",  limit, offset];
